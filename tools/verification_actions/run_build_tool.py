#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run a build verification command in the workspace."""
...
class RunBuildTool(BaseTool):
    ...
    def definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="run_build",
            description="在 workspace 内执行 build 验证命令。",
            input_schema={
                "type": "object",
                "properties": {
    ...
    def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        command = input_data.get("command") or ["cmake", "--build", "build"]
        if isinstance(command, str):
            command = shlex.split(command)
        approved = isinstance(input_data.get("_runtime_authorization"), RuntimeAuthorization)
        provider = CommandVerificationProvider(list(command), modes=["build", "auto"], approved=approved)
