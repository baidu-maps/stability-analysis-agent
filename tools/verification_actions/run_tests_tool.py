#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run a test verification command in the workspace."""
...
class RunTestsTool(BaseTool):
    ...
    def definition(self) -> ToolDefinition:
        return ToolDefinition(
            name="run_tests",
            description="在 workspace 内执行 test 验证命令。",
            input_schema={
                "type": "object",
                "properties": {
    ...
    def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        command = input_data.get("command") or ["python3", "-m", "pytest"]
        if isinstance(command, str):
            command = shlex.split(command)
        approved = isinstance(input_data.get("_runtime_authorization"), RuntimeAuthorization)
        provider = CommandVerificationProvider(list(command), modes=["test", "auto"], approved=approved)
