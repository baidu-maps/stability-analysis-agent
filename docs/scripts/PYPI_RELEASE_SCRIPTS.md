...
pip install -i https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple "stability-analysis-agent[rag]==1.3.4"
sa-agent --help
```
...
| 触发 | 行为 |
|------|------|
| 推送 tag `v*`（如 `v1.3.4`） | 跑确定性套件 → 构建 → 上传 **正式 PyPI** |
