#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""分析报告（``cli_reports``）与本地缓存的路径与占用工具。

提供设置菜单「清理本地缓存」所依赖的所有纯函数：

- :func:`report_root`             返回当前工作目录下报告根
- :func:`format_bytes`            字节数 -> 人类可读字符串
- :func:`summarize_cli_reports`   统计 ``cli_reports/`` 下报告数与占用
- :func:`clear_cli_reports`       清空 ``cli_reports/`` 中的内容
- :func:`print_cli_reports_overview`  控制台概览输出（设置菜单调用）

报告根目录解析顺序：

1. 环境变量 ``STABILITY_AGENT_REPORT_DIR``；
2. 当前工作目录下的 ``./cli_reports``。
"""
...
def report_root() -> Path:
    ...
    return (Path.cwd() / "cli_reports").resolve()
...
def _list_report_dirs(root: Path) -> List[Path]:
    """列出 ``root`` 下所有形如 ``YYYYMMDD_HHMMSS_*`` 的分析会话目录。"""
...
def summarize_cli_reports(*, preview_limit: int = 5, root: Optional[Path] = None) -> Dict[str, Any]:
...
def clear_cli_reports(
    *,
    ...
    Args:
        root: 报告根目录，默认 :func:`report_root`。
        only_preview: 是否仅清空 ``summarize_cli_reports`` 中标记为 preview
            的会话（最近 ``preview_limit`` 个）。``False`` 时清空全部。
        preview_limit: 当 ``only_preview=True`` 时挑选的最新会话数量。
    """
    target = (root or report_root()).resolve()
    stats = summarize_cli_reports(root=target, preview_limit=preview_limit if preview_limit > 0 else 1)
    targets: List[Path]
    if not stats["exists"]:
        return {
            "path": str(target),
